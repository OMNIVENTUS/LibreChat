...
export default function AgentPanel() {
  ...
  const canEditAgent = useMemo(() => {
    ...
    if (agentQuery.data?.isCollaborative ?? false) {
      return true;
    }

    return canEdit ?? (agentQuery.data?.author === user?.id || user?.role === SystemRoles.MANAGER);
  }, [agentQuery.data?.id, agentQuery.data?.isCollaborative, agentQuery.data?.author, user?.role, user?.id, canEdit]);
