...
export default function UserPanel() {
  ...
  const handleUpdateRole = async () => {
    try {
      ...
      showToast({
        message: localize('com_users_role_update_success'),
        severity: NotificationSeverity.SUCCESS,
      });
      setIsRoleDialogOpen(false);
    } catch (error) {
      showToast({
        message: localize('com_users_role_update_error'),
        severity: NotificationSeverity.ERROR,
      });
    }
  };
  ...
  const openRoleDialog = (user: any) => {
    setSelectedUser(user);
    setSelectedRole(user.role || SystemRoles.USER);
        ...
        <DialogContent className="sm:max-w-[425px] text-text-primary p-4">
          <DialogHeader>
            <DialogTitle className="flex items-center">
              ...
              {localize('com_users_update_role')}
            </DialogTitle>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              <h4 className="font-medium break-all">{selectedUser?.username || selectedUser?.email}</h4>
              <p className="text-sm text-muted-foreground break-words">
                {localize('com_users_current_role')}:{' '}
                {selectedRole || localize('com_users_no_role')}
              </p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">{localize('com_users_select_role')}</label>
              <SelectDropDown
                value={selectedRole}
                availableValues={roleOptions}
                setValue={handleRoleChange}
                showAbove={true}
                emptyTitle={true}
                placeholder={localize('com_users_select_role_placeholder')}
              />
            </div>
          </div>
          <DialogFooter className="flex-col space-y-2 sm:flex-row sm:justify-end sm:space-x-2 sm:space-y-0">
