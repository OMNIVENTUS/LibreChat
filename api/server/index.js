...
// [OMNIVENTUS-START] business actions + shared-files RAG preload bootstraps
...
const startServer = async () => {
  ...
  // [OMNIVENTUS-START] register business-action providers; preload shared files
  // into the RAG vector store (File model is tenant-isolated → runAsSystem).
  initBusinessActions();
  runAsSystem(preloadFiles).catch((err) => {
    logger.error('[preloadFiles] Background preload failed:', err);
  });
  // [OMNIVENTUS-END]
  ...
  // [OMNIVENTUS] user-update endpoint for the user-admin feature
  // (listing/deletion go through the native /api/admin/users API)
