...
const {
  ...
  FileSources,
  ResourceType,
  // [OMNIVENTUS] role checks for shared-files scope filtering
  SystemRoles,
  EModelEndpoint,
  PermissionBits,
  checkOpenAIStorage,
...
router.get('/', async (req, res) => {
  try {
    ...
    // [OMNIVENTUS-START] shared/preloaded files: include public + shared files
    // the user can access (by group, role, or id) alongside their own files.
    const accessGroups = [
      ...(req.user?.file_access_groups || []),
      req.user?.role,
      req.user?.id,
    ].filter(Boolean);
    const scopeOr = [
      { user: req.user.id },
      { scope: 'public' },
      { scope: 'shared', access_control: { $in: accessGroups } },
    ];
    if (req.user?.role === SystemRoles.ADMIN || req.user?.role === SystemRoles.MANAGER) {
      scopeOr.push({ scope: 'shared' });
    }
    const files = await db.getFiles({ $or: scopeOr });
    // [OMNIVENTUS-END]
...
router.delete('/', async (req, res) => {
  try {
    ...
    // [OMNIVENTUS] admins may delete files they do not own (shared/preloaded files)
    if (nonOwnedFiles.length > 0 && req.user.role === SystemRoles.ADMIN) {
      ownedFiles.push(...nonOwnedFiles);
      nonOwnedFiles.length = 0;
    }
